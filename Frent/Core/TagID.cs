...
{
    internal TagID(ushort id) => Index = id;
    internal readonly ushort Index;

    /// <summary>
    /// The type that this TagID represents
    /// </summary>
    public Type Type => Tag.TagTable[Index];
    /// <summary>
    ...
    public readonly bool Equals(TagID other) => other.Index == Index;
    /// <summary>
    /// Checks if this TagID instance represents the same type as <paramref name="other"/>
    /// </summary>
    /// <param name="other">The tag to compare against</param>
    /// <returns><see langword="true"/> when they represent the same type, <see langword="false"/> otherwise</returns>
    public override bool Equals(object? other) => other is TagID t && Index == t.Index;
    /// <summary>
    /// Checks if two <see cref="TagID"/>s represent the same type
    /// </summary>
    /// <returns><see langword="true"/> when they represent the same type, <see langword="false"/> otherwise</returns>
    public static bool operator ==(TagID left, TagID right) => left.Index == right.Index;
    /// <summary>
